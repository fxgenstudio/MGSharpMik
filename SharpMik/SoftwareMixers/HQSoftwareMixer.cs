		...
		protected override uint WriteSamples(sbyte[] buf, uint todo)
		{
			...
			while (todo != 0)
			{
				if (m_TickLeft == 0)
				{
					...
					m_TickLeft = (ModDriver.MixFreq * 125 * SAMPLING_FACTOR) / (ModDriver.Bpm * 50);
				...
				while (left != 0)
				{
					...
					for (t = 0; t < m_VcSoftChannel; t++)
					{
						...
						{
							long val = (((long)m_CurrentVoiceInfo.Frequency) << (FRACBITS - SAMPLING_SHIFT)) /  ModDriver.MixFreq;
							m_CurrentVoiceInfo.CurrentIncrement = val;
